...
On Cloud the **Auto-detect** button in the settings dialog is enough. It queries
...
The six status fields in the settings dialog start out empty. You find your own
...
On first start, press `S` for settings and configure - where token and
field ID come from is covered under [Prerequisites](#prerequisites):
- Jira host URL (Cloud: the canonical `https://your-company.atlassian.net`)
- Token — Jira Cloud: an API token, Data Center: a personal access token
