...
def _mixed_timesheet() -> Timesheet:
    ...
    jira = WorklogEntry(date=date(2026, 7, 23), ticket="PROJ-1001", summary="Agile", author="M", budget="", hours=1.0)
    manual = ManualEntry(
...
async def test_manual_dialog_keeps_unknown_customer_when_editing() -> None:
    ...
    entry = ManualEntry(entry_id=3, entry_date=date(2026, 7, 1), ticket="PROJ-1004", hours=8.0, customer="Altkunde")
...
async def test_edit_dialog_has_delete_button() -> None:
    ...
    entry = ManualEntry(entry_id=3, entry_date=date(2026, 7, 1), ticket="PROJ-1004", hours=8.0)
...
async def test_save_returns_result_without_delete_flag() -> None:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        await pilot.pause()
        app.screen.query_one("#manual-ticket", Input).value = "PROJ-1004"
