...
<#
.SYNOPSIS
    Compiles jira-timesheet into a standalone Windows binary with Nuitka.

.DESCRIPTION
    Produces a self-contained --standalone build (no Python install needed on
    the target machine). Output: dist\jira-timesheet\jira-timesheet.exe plus its
    DLLs, and a zipped dist\jira-timesheet-vX.Y.Z-win64.zip ready to hand out.
#>
...
$root    = $PSScriptRoot
$entry   = Join-Path $root "src\jira_timesheet\__main__.py"
$initPy  = Join-Path $root "src\jira_timesheet\__init__.py"
$outDir  = Join-Path $root "dist"
$distDir = Join-Path $outDir "jira-timesheet"
...
Write-Host "Compiling jira-timesheet v$version with Nuitka..." -ForegroundColor Cyan
...
if (Test-Path $nuitkaDist) { Rename-Item -Path $nuitkaDist -NewName "jira-timesheet" }

$elapsed = [int]((Get-Date) - $started).TotalSeconds
$exe     = Join-Path $distDir "jira-timesheet.exe"
...
# der Empfaenger entpackt also direkt einen sauberen jira-timesheet-Ordner
$zip = Join-Path $outDir "jira-timesheet-v$version-win64.zip"
